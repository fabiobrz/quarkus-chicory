...
/**
 * A convenient class to manage an application Wasm modules - both statically configured and dynamically loaded -
 * at runtime.
 */
public class Wasms {
    ...
    Wasms(List<Wasm> wasms) {
        for (Wasm wasm : wasms) {
            Log.info("A configured Wasm module " + wasm.getName() + " will be added");
    ...
    public Wasm update(final Wasm wasm) {
        if (!storage.containsKey(wasm.getName())) {
            throw new IllegalArgumentException(
                    "Cannot find the " + wasm.getName() + "Wasm module that should be updated.");
        }
        Wasm existing = storage.remove(wasm.getName());
        storage.put(existing.getName(), wasm);
        return existing;
    }
    ...
    static Wasm createStatic(final String key, final ChicoryConfig.ModuleConfig config) {
        ...
        return new Wasm(key, wasmModule, ExecutionMode.Interpreter, new StaticInstanceProvider());
    }
